...
from app.crud.crud_prestamo import (
    ...
    get_prestamos_by_usuario, devolver_prestamo
)
...
def devolver_ejemplar(
    prestamo_id: int,
    copia_id: Optional[int] = None,
    db: Session = Depends(get_db),
    current_user = Depends(require_any_role(["Administrador", "Usuario"]))
):
    """Endpoint para devolver uno o todos los ejemplares de un préstamo."""
    return devolver_prestamo(db=db, prestamo_id=prestamo_id, copia_id=copia_id)